...
function coinEarningOverview() {
  ...
  return `<details class="coin-verdienst special-dex-wide-card" data-coin-earning-overview>
    <summary><span><b>So verdienst du CAPCOINS</b><small>Vergütung anzeigen</small></span>${materialIconMarkup('chevron_right')}</summary>
    <div class="coin-verdienst-inhalt">
      ...
      <div class="coin-verdienst-frei"><span>✨</span><p><b>Freie Routine</b><small>Beim Anlegen selbst zwischen 0 und 50 CAPCOINS festlegen. Standard: 5 CAPCOINS.</small></p></div>
    </div>
  </details>`;
}
...
function rewardEditor({ userId, existing = null, onSaved }) {
  ...
  backdrop.innerHTML = `<section class="kategorie-sheet special-dex-sheet coin-editor" role="dialog" aria-modal="true" aria-label="Belohnung ${existing ? 'bearbeiten' : 'anlegen'}">
    ...
    <form data-coin-reward-form>
      ...
      <label class="dex-entry-field"><span>Preis in CAPCOINS</span><input class="input coin-zahlenfeld" data-reward-cost type="number" inputmode="numeric" min="1" max="100000" value="${existing?.cost || 100}" required></label>
...
function rewardMarkup(item, balance) {
  ...
  return `<article class="coin-reward" data-reward-id="${item.id}">
    <div class="coin-reward-kopf">${muscleCoinMarkup('coin-reward-symbol')}<span><b>${escapeHtml(item.name)}</b><small>${item.cost} CAPCOINS</small></span><strong>${percent}%</strong></div>
...
export async function mountCoinDex(container, { userId, signal, mountChrome }) {
  ...
  container.innerHTML = `<div class="wrap pad-bottom coin-dex-seite"><div class="coin-dex-inhalt special-dex-content special-dex-stack"><div class="daten-laden">CAPCOINS werden geladen …</div></div></div>`;
  ...
  container.querySelector('.coin-dex-inhalt').innerHTML = `
    <section class="coin-balance-card special-dex-hero">
      ...
      <span><small>DEIN KONTOSTAND</small><strong>${balance}</strong><b>CAPCOINS</b></span>
    </section>
    ${coinEarningOverview()}
    ${next ? `<section class="coin-next special-dex-wide-card"><span><b>Nächste Belohnung</b><small>${escapeHtml(next.name)} · ${next.cost} CAPCOINS</small></span><strong>${Math.max(0, next.cost - balance)} fehlen</strong><div class="coin-progress"><i style="width:${progress}%"></i></div></section>` : ''}
  ...
  container.querySelector('.coin-reward-list').onclick = async (event) => {
    ...
    if (!confirm(`„${reward.name}“ für ${reward.cost} CAPCOINS einlösen?`)) return;
