...
function bodyHeroMarkup(state) {
  ...
  return `<div class="body-v2-stack ${SPECIAL_DEX_CLASSES.content} ${SPECIAL_DEX_CLASSES.stack}"><section class="body-v2-hero ${SPECIAL_DEX_CLASSES.hero}" style="--body-progress:${progress}%">
    ...
    <button class="body-analysis-info" type="button" aria-expanded="false" aria-label="BODY-Auswertung erklären">i</button>
  </section>
  <div class="body-analysis-help" hidden>
    <p>Der <b>BODY</b> bewertet nicht einzelne Tageswerte, sondern deinen geglätteten Gewichtsverlauf.</p>
...
export async function mountBodyMetrics(container, { session, profile, onProfileUpdated, signal, onRendered }) {
  ...
  const renderOnce = async () => {
    ...
    // (Update-Hinweis mit eigenen BODY-Notizen) wieder anzuhängen und
  ...
  const bindEntryOverlay = (overlay) => {
    const closeAndRender = async () => {
      overlay.remove();
      try { await render(); }
      catch (error) { toast(error.message || 'BODY konnte nicht aktualisiert werden'); }
    };
  ...
  container.innerHTML = '<div class="wrap"><section class="card"><p>BODY wird geladen …</p></section></div>';
  try {
    await render();
  } catch (error) {
    if (!signal?.aborted) container.innerHTML = `<div class="wrap"><section class="card"><p class="msg err">BODY konnte nicht geladen werden.<br><small>${escapeHtml(error.message)}</small></p></section></div>`;
  }
