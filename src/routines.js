  ...
  backdrop.innerHTML = `<section class="kategorie-sheet special-dex-sheet routine-editor${selectedTemplate === 'custom' ? '' : ' routine-template-editor'}" role="dialog" aria-modal="true" aria-label="Routine ${existing ? 'bearbeiten' : 'hinzufügen'}">
    ...
    <form data-routine-form>
      ...
      ${selectedTemplate === 'custom'
        ? `<label class="dex-entry-field"><span>CAPCOINS pro Abschluss</span><input class="input coin-zahlenfeld" data-routine-coins type="number" inputmode="numeric" min="0" max="50" value="${existing?.coin_reward ?? 5}" required><small class="routine-coin-info">Für diese freie Routine selbst festlegen: 0–50 CAPCOINS.</small></label>`
